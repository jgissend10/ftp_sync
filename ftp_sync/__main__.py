...
@click.option(
    ...
    "--config-file",
    type=str,
    default=FTP.FTP_SYNC_CONFIG_PATH,
...
def main(ctx, config_file, debug):
    ...
    if os.path.exists(config_file):
        ...
        logger.error(
            f"Config file does not exist. Create one at {FTP.FTP_SYNC_CONFIG_PATH} or pass one with the -c/--config option. Checkout https://github.com/branflakes2/ftp_sync/blob/master/ftp_sync.yaml for an example."
        )
